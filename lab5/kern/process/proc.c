...
load_icode(unsigned char *binary, size_t size)
{
    ...
    for (; ph < ph_end; ph++)
    {
        ...
        if (start < la)
        {
            ...
            /* page must be the last allocated page from the TEXT/DATA loop */
            if (page == NULL)
            {
                goto bad_cleanup_mmap;
            }
...
typedef int (*user_test_fn_t)(void *);

#define DEF_USER_TEST(name)                               \
static int user_test_##name(void *arg)                    \
{                                                         \
    KERNEL_EXECVE(name);                                  \
    panic(#name " execve failed.\n");                     \
}
