...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (i = 0; i < elf.e_phnum; i++)
    {
        ...
        while (start < end)
        {
            ...
            if ((ret = load_icode_read(fd, buffer, to_read, offset)) != 0)
            {
                goto bad_pgdir_cleanup_mm;
            }
            if ((ret = copy_to_user_pages(mm->pgdir, start, buffer, to_read, perm)) != 0)
            {
                goto bad_pgdir_cleanup_mm;
            }
            start += to_read;
